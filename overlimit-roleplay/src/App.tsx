...
export default function App() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('open-rules', handleOpenRules);
    return () => window.removeEventListener('open-rules', handleOpenRules);
  }, []);

  useEffect(() => {
    const hash = window.location.hash;
    if (hash.includes('admin=true')) {
      ...
      if (idParam && idParam.startsWith('portal-')) {
        setAdminDocId(idParam.replace('portal-', ''));
      }
    }
  }, []);
  ...
  if (adminCategory || adminDocId) {
    return (
      <div className="min-h-screen bg-dark-black font-sans text-white overflow-x-hidden selection:bg-light-cyan selection:text-dark-black dot-grid">
        <AdminPortal docId={adminDocId} category={adminCategory} />
      </div>
    );
  }
