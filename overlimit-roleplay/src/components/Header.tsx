          ...
          {[
            ...
            <a 
              ...
              onClick={(e) => {
                if (item.name === 'Rules') {
                  e.preventDefault();
                  window.dispatchEvent(new CustomEvent('open-rules'));
                }
              }}
              className={`font-technical text-[11px] uppercase tracking-[0.3em] font-black transition-colors ${item.name === 'Home' ? 'text-light-cyan border-b-2 border-light-cyan pb-1' : 'text-soft-gray hover:text-light-cyan'}`}
