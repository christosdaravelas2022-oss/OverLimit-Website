...
const GREEK_QUESTIONS: Record<string, { label: string; placeholder: string; type: 'text' | 'textarea' | 'number' }[]> = {
  ...
  'Δημαρχείο': [
    ...
    { label: 'Στόχος σας στο Δικαστικό Σώμα/Δημαρχείο', placeholder: 'Πώς θα βοηθήσετε στην απονομή δικαιοσύνης;', type: 'textarea' },
  ],
  'Ownership': [
...
function ApplicationModal({ job, onClose }: ApplicationModalProps) {
  ...
  const lastSubmitTime = React.useRef<number>(0);

  const handleInputChange = (field: string, value: string) => {
    // Limit input length to prevent massive payloads
  ...
  const handleLoginAndSubmit = async (e: React.FormEvent) => {
    ...
    // Client-side rate limiting (1 request per 10 seconds)
    const now = Date.now();
    if (now - lastSubmitTime.current < 10000) {
      setErrorMsg("Παρακαλώ περιμένετε πριν προσπαθήσετε ξανά.");
      return;
    }
    lastSubmitTime.current = now;

    if (honeypot.length > 0) {
      setErrorMsg("Σφάλμα συστήματος (SPAM-PREV).");
      return;
    }
    if (!captchaPassed) {
      setErrorMsg("Παρακαλώ επιβεβαιώστε ότι είστε άνθρωπος λύνοντας το CAPTCHA.");
      return;
    }

    // Payload size and Discord Tag format validation
    if (formData.discordTag && (formData.discordTag.length < 2 || formData.discordTag.length > 32)) {
      setErrorMsg("Το Discord Tag πρέπει να είναι από 2 έως 32 χαρακτήρες.");
      return;
    }
    let totalLength = 0;
    ...
    if (totalLength > 5000) {
       setErrorMsg("Η αίτηση είναι πολύ μεγάλη. Παρακαλώ συντομεύστε τις απαντήσεις σας.");
       return;
    }
    ...
    if (!user) {
      try {
        ...
        setErrorMsg("Συνέβη ένα σφάλμα ασφαλείας. Ανανεώστε τη σελίδα και δοκιμάστε ξανά.");
        return;
      }
    }
    
    // Check again, since signInAnonymously might have succeeded and updated state
  ...
  const handleSubmit = async (currentUser: User) => {
    ...
    const policeWebhook = "FIXME_POLICE_APPLICATIONS";
    const dimarxeioWebhook = "FIXME_DIMARXEIO_APPLICATIONS";
    ...
    const webhookUrl = 
      ...
      job.title === 'Ελληνική Αστυνομία' ? policeWebhook :
      job.title === 'Δημαρχείο' ? dimarxeioWebhook :
      defaultWebhook;
    let submissionSuccess = true;
    let docId = "";

    try {
      // Allow applying again after 7 days
      const q = query(
        collection(db, "applications"), 
        where("applicantUid", "==", currentUser.uid)
      );
      
      const appRef = await getDocs(q);
      const apps = appRef.docs.map(d => d.data()).filter((d: any) => d.jobTitle === job.title);
      // Find the most recent application
      const recentApp = apps.sort((a, b) => (b.createdAt?.toMillis?.() || 0) - (a.createdAt?.toMillis?.() || 0))[0];
      
      if (recentApp && recentApp.createdAt) {
        const _7DaysInMillis = 7 * 24 * 60 * 60 * 1000;
        const nowMillis = Date.now();
        const appMillis = recentApp.createdAt.toMillis();
        
        if (nowMillis - appMillis < _7DaysInMillis) {
          setErrorMsg("Έχετε ήδη υποβάλει αίτηση πρόσφατα! Μπορείτε να κάνετε μόνο μία αίτηση κάθε 7 ημέρες.");
      ...
      if (error && error.message && error.message.includes('Missing or insufficient permissions')) {
        setErrorMsg("Έχετε ήδη υποβάλλει αίτηση για αυτή τη θέση!");
      } else {
        setErrorMsg("Συνέβη ένα σφάλμα. Παρακαλώ δοκιμάστε ξανά.");
      }
      try {
  ...
  const getDiscordWebhookPayload = (job: any, fields: any[], docId: string) => {
    const baseUrl = typeof window !== 'undefined' ? window.location.origin : 'https://over-limit-website.vercel.app';
    const adminUrl = `${baseUrl}/#admin=true&category=${encodeURIComponent(job.title)}&id=portal-${docId}`;
    return {
      embeds: [{
          ...
          {isSuccess ? (
            ...
            <>
              ...
              <form onSubmit={handleLoginAndSubmit} className="space-y-6">
                ...
                {/* Honeypot field - AI/bots often fill visually hidden fields */}
                ...
                {/* Real Canvas CAPTCHA */}
                <div className="pt-2 pb-2 border-t border-white/10 mt-4">
                  <CanvasCaptcha onVerify={setCaptchaPassed} />
                </div>

                {errorMsg && (
                  <div className="bg-red-500/10 border border-red-500/30 text-red-500 p-4 rounded-sm text-sm font-bold text-center">
                    {errorMsg}
                  </div>
                )}
                <motion.button
...
export function CanvasCaptcha({ onVerify }: { onVerify: (isValid: boolean) => void }) {
  ...
  const drawCaptcha = React.useCallback((text: string) => {
    ...
    // Draw text with some rotation to make OCR harder
  ...
  return (
    <div className="flex flex-col gap-2">
      <label className="font-technical text-[10px] text-soft-gray uppercase tracking-widest block font-black">
        Επιβεβαίωση Ανθρώπου (CAPTCHA) - Προστασία AI
      </label>
      <div className="flex gap-2 items-center">
        <canvas ref={canvasRef} width="120" height="40" className="border border-white/20 rounded-sm" />
        <button 
          type="button" 
          onClick={() => { const c = generateCode(); setCode(c); drawCaptcha(c); setInputCode(''); onVerify(false); }}
          className="text-[10px] p-2 tracking-widest uppercase font-black text-soft-gray hover:text-white transition-colors"
        >
          Ανανέωση
        </button>
      </div>
      <input 
        ...
        className="w-full bg-dark-black border border-white/10 px-4 py-3 text-white font-sans text-sm focus:border-light-cyan focus:outline-none transition-colors rounded-sm font-mono tracking-[0.2em]"
        placeholder="ΕΙΣΑΓΕΤΕ ΤΟΝ ΚΩΔΙΚΟ"
        maxLength={5}
        required
      />
    </div>
